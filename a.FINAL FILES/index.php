<?php session_start(); ?>

<!DOCTYPE html>
<html>
  <head>
    <title>Digitagrize it</title>
    <link rel="icon" href="images/icons/favicon.png" />
    <meta charset="UTF-8" />
    <meta
      name="viewport"
      content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0"
    />
    <meta http-equiv="X-UA-Compatible" content="ie-edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    
    <link rel="stylesheet" href="about.css">
    <link rel="stylesheet" href="faq.css">
    <link rel="stylesheet" href="main.css" />
    <link rel="stylesheet" href="contact.css" />
    <link rel="stylesheet" href="homepage.css" />
    <link rel="stylesheet" href="navigation.css" />
    <link rel="preconnect" href="https://fonts.googleapis.com" />
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
    <link
      href="https://fonts.googleapis.com/css2?family=Montserrat:wght@100&display=swap"
      rel="stylesheet"
    />
    <link
      rel="stylesheet"
      href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css"
    />
    <script src="script.js" defer></script>
    
  </head>
  <body>
    <?php include("navigation.php"); ?>
    <div class="google-s">
    <form action="https://www.google.com/search" method="GET">
        <input type="text" name="q" placeholder="Google Search">
        <input class="button-s" type="submit" value="Search">
    </form>
    </div>
    <section id="home">
      <script
        language="javascript"
        type="text/javascript"
        src="home.txt"
      ></script> 
    </section>
      
    <section class="about-area" id="about">
      <div class="wrapper">
        <div>
            <img class="image" src="https://images.unsplash.com/photo-1627920769541-daa658ed6b59?ixlib=<rb-></rb-        >1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=2233&q=80" />
        </div>
    <div>
        <div class="text">
            <h2 style="font-size: 2.75vw;"> Who are we</h2>
            <p style="font-size: 1.75vw;">
               <b>Digitagrize it</b> aims to help the agricultural world market grow and be more accesible through this new digital platform
            </p>
        </div>
    </div>
    <div>
        <div class="text">
            <h2 style="font-size: 2.75vw;"> Production</h2>
            <p style="font-size: 1.75vw;">
                  Provide your production and make sure it reaches more and more clients
            </p>
          </div>
    </div>
    <div>
      <img class="image" src="https://images.unsplash.com/photo-1464226184884-fa280b87c399?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1770&q=80" />
    </div>
    <div>
      <img class="image" src="https://images.unsplash.com/photo-1518620664663-35a1f6378ec6?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1738&q=80" />
    </div>    
    <div>
        <div class="text">
            <h2 style="font-size: 2.75vw;"> Land</h2>
            <p style="font-size: 1.75vw;">
                Find an associate for your land and make a new cooperative business plan  
            </p>
        </div>
    </div>
        <div>
        <div class="text">
            <h2 style="font-size: 2.75vw;"> Data and Information</h2>
            <p style="font-size: 1.75vw;">
                We collect and analyze data and content relative to the global agricultural market, creating Information that may lead to business opportunities 
            </p>
        </div>    
    </div>
        <div>
      <img class="image" src="https://images.unsplash.com/photo-1579705745811-a32bef7856a3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1770&q=80" />
    </div>
            <div>
      <img class="image" src="https://images.unsplash.com/photo-1545987796-200677ee1011?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1770&q=80" />
    </div>
        <div>
        <div class="text">
            <h2 style="font-size: 2.75vw;"> Network</h2>
            <p style="font-size: 1.75vw;">
                We aim to build a global network with trustworthy and reliable members that make transactions quickier and easier 
            </p>
        </div>
    </div>
      <div>
        <div class="text">
            <h2 style="font-size: 2.75vw;"> Transaction Safety</h2>
            <p style="font-size: 1.75vw;">
                We provide any means necessary to ensure that every transaction between different parties safe and secure  
            </p>
        </div>
    </div>  
      <div>
        <img class="image" src="https://images.unsplash.com/photo-1614064850003-13dbfd69fd11?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1770&q=80" />
      </div> 
    </div>
    </section>

    <section class="faq-area" id="faq">
        <section class="faq-container">
          <br><br><br><br><br><br><br><br>
          <h1 class="faq-heading" align="center" style="font-size:50px;">FAQ</h1>
              <h1 class="faq-page">Can i sell my property through your platform?</h1>
              <div class="faq-body">
                  <p>No, 'Digitagrize it' is not a real estate platform. As far as land is concerned, our main purpose is to help owners find an associate, make a business plan and cultivate the land, sharing the fruit of this effort  </p>
              </div>
          <hr class="hr-line">
              <h1 class="faq-page">Is Digitagrize It easy to use?</h1>             
              <div class="faq-body">
                  <p>Yes! All you have to do is create an account and after that you can easily submit your listings in the click of a button.
                  </p>
              </div>
          <hr class="hr-line">
              <h1 class="faq-page">Does Digitagrize It show my information to other users when they see my listings?</h1>
              <div class="faq-body">
                  <p>Digitagrize It only shares the information you allow it to. In each submission you make you can add the contact information you want other users to contact you with. Digitagrize It does not share any of the information added during your registration.</p>
                </div>
          <hr class="hr-line">
          </br></br></br></br>
    </section>
    <script src="faq.js"></script>
    </div>
    </section>

    <section class="contact-area" id="contact">
    
        <div class="container">
          <div><br>
          <h1 align="center">Contact Us</h1></div>
          <div class="mail">
            <label for="myFN">First Name</label>
            <input type="text" id="myFN" required />
            <label for="myLN">Last Name</label>
            <input type="text" id="myLN" required />
            <label for="comp">To</label>
            <input type="text" value="company@mail.com" id="comp" disabled />
            <label for="myMAIL">From</label>
            <input type="text" id="myMAIL" required />
            <label for="fname">Subject</label>
            <textarea
              id="subject"
              name="subject"
              placeholder="Write something.."
              style="height: 200px"
            ></textarea>
            <button class="button" href="#">Submit</button>
          </div><br>
        </div>
      </div>
    </section>

    <div class="footer">
      <p>Find Us On:</p>
      <a href="#" class="fa fa-facebook"></a>
      <a href="#" class="fa fa-twitter"></a>
      <a href="#" class="fa fa-linkedin"></a>
      <p>@digitagrize.it.eu 2022<br /></p>
    </div>

    <script>
    function checkInput() {
        var query = document.getElementById('search').value;
        window.find(query);
        return true;
      }
    </script>
    
    <script>
      $(document).ready(function() {
	$(".fa-search").click(function() {
	   $(".togglesearch").toggle();
	   $("input[type='text']").focus();
	 });
});
</script>

  </body>
</html>
